...
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
GAME_MENU, GAME_RUNNING, GAME_OVER = range(3)
...
class Game():
    ...
    def show_menu(self):
        ...
        menu_text = font.render('Press SPACE to start/continue', True, WHITE)
        self.screen.blit(menu_text, menu_text.get_rect(center=(WIDTH // 2, HEIGHT // 2)))
    def show_game_over(self):
        font = pygame.font.SysFont('comicsansms', 40)
        losetext = font.render('Game Over', True, (255, 255, 255))
        lospoints = font.render(f'Score: {self.ball.score}', True, (255, 255, 255))
        wintext = font.render('You win yay', True, (0, 0, 0))
        
        if self.ball.ball.bottom > HEIGHT:
            self.screen.fill((0, 0, 0))
            self.screen.blit(losetext, losetext.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 75)))
            self.screen.blit(lospoints, lospoints.get_rect(center=(WIDTH // 2, HEIGHT // 2)))
            pygame.display.flip()
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # fix GAME OVER -> ESC -> SPACE ->> GAME_OVER again
    ...
    def run(self):
        while True:
            ...
            if self.game_state == GAME_MENU:
                self.show_menu()
            elif self.game_state == GAME_RUNNING and self.game_state != GAME_OVER:
                self.update_game()
                self.draw_game()
                self.show_game_over()
            pygame.display.flip()
