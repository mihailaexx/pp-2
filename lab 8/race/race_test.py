...
class Enemy(pygame.sprite.Sprite): # use class for game objects
    def __init__(self):
        ...
        self.rect.center=(random.randint(0,size[0]),-car_h) # randomly create an enemy
...
class Coin(pygame.sprite.Sprite): # class for coin
    ...
    def spawn(self): # method for creating a coin
        self.rect.center = (random.randint(25, size[0]-25), random.randint(25, size[1]-25))
...
SPEED = 15
cars = 0 # score for cars
coins = 0 # score for coins
player = MainCar() # creating variables
enemy = Enemy()
coin = Coin()
...
while not done:
    ...
    for entity in all_sprites: # spawning and drawing a cars
        screen.blit(entity.image, entity.rect)
        entity.move()

    screen.blit(coin.image, coin.rect) # creating coin
    if pygame.sprite.collide_rect(player, coin): # cheking for collecting coin
        coin.kill()
        coin.spawn()
        coins += 1
    ...
    if pygame.sprite.spritecollideany(player, all_enemies): # endgame screen will appear after collision of main car and enemies contact 
        ...
        screen.blit(go_font.render(f"Coins: {coins}", True, (0, 0, 0)), (120, 590))
        pygame.display.flip()
        sleep(2) # screen will displaying 2 sec
        done = True
    
    screen.blit(font.render(f"Cars: {cars}", True, (0, 0, 0)), (10, 10))
    screen.blit(font.render(f"Coins: {coins}", True, (0, 0, 0)), (10, 30))
    pygame.display.flip()
